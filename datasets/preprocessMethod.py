...
def preprocess(opt):
    ...
    if opt.EvalEOSTestOnly.lower() == 'false':
        bEvalEOSTestOnly = False
    else:
    ...
    printDebug(
        str(tra_sess[:3]) + "the first 3 sessions that will be used for train - out of [" + str(len(tra_sess)) + "]")
    printDebug(
        str(tes_sess[:3]) + "the first 3 sessions that will be used for test - out of [" + str(len(tes_sess)) + "]")
    printDebug(
        "-- Splitting train set and test set @ %ss" % datetime.datetime.now())  # the Split us done by the session id
    ...
    if opt.dataset == 'diginetica':
        pathBase = "./../datasets/diginetica"
        if not os.path.exists(pathBase + pathExt):
            os.makedirs(pathBase + pathExt)
        pickle.dump(tra, open(pathBase + pathExt + '/train.txt', 'wb'))
        pickle.dump(tes, open(pathBase + pathExt + '/test.txt', 'wb'))
        pickle.dump(tra_seqs, open(pathBase + pathExt + '/all_train_seq.txt', 'wb'))  # not in use
    elif opt.dataset == 'yoochoose':
        pathBase = "./../datasets/yoochoose"
        if not os.path.exists(pathBase + '1_4' + pathExt):
            os.makedirs(pathBase + '1_4' + pathExt)
        if not os.path.exists(pathBase + '1_64' + pathExt):
            os.makedirs(pathBase + '1_64' + pathExt)
        pickle.dump(tes, open(pathBase + '1_4' + pathExt + '/test.txt', 'wb'))
        pickle.dump(tes, open(pathBase + '1_64' + pathExt + '/test.txt', 'wb'))
        ...
        pickle.dump(tra4, open(pathBase + '1_4' + pathExt + '/train.txt', 'wb'))
        pickle.dump(seq4, open(pathBase + '1_4' + pathExt + '/all_train_seq.txt', 'wb'))  # not in use

        pickle.dump(tra64, open(pathBase + '1_64' + pathExt + '/train.txt', 'wb'))
        pickle.dump(seq64, open(pathBase + '1_64' + pathExt + '/all_train_seq.txt', 'wb'))  # not in use

    else:
        pathBase = "./../datasets/sample"
        if not os.path.exists(pathBase + pathExt):
            os.makedirs(pathBase + pathExt)
        pickle.dump(tra, open(pathBase + pathExt + '/train.txt', 'wb'))
        pickle.dump(tes, open(pathBase + pathExt + '/test.txt', 'wb'))
